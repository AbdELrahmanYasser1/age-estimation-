...
# fiting for missing data in balance and EstimatedSalary of client 
# handeling the outliers
""" hena lama 3mlnaha b el 'mean' elnatega kanet 3alia 2wy 3n ba2y elntaig
 3lshan keda estkhdemna el 'most_frequent'"""
...
#########################################################################################
# Logistic Regression
"""matensash en fe el logistic regression laaaazem t3mel feature scaling """
# fitting Logistic Regression to the training model 

from sklearn.linear_model import LogisticRegression
classifier = LogisticRegression( penalty= 'elasticnet',l1_ratio=0.9, C = 1.0 , fit_intercept = True , 
...
clsfier_probs = classifier.predict_proba(X_test)

rf_probs = rf_probs[:, 1]
clsfier_probs = clsfier_probs[:, 1]

from sklearn.metrics import roc_curve, roc_auc_score

r_auc = roc_auc_score(y_test, r_probs,multi_class='ovo')
rf_auc = roc_auc_score(y_test, rf_probs)
clsfier_auc = roc_auc_score(y_test, clsfier_probs)
print('Random (chance) Prediction: AUROC = %.3f' % (r_auc))
print('Random Forest: AUROC = %.3f' % (rf_auc))
print('LR : AUROC = %.3f' % (clsfier_auc))

r_fpr, r_tpr, _ = roc_curve(y_test, r_probs)
rf_fpr, rf_tpr, _ = roc_curve(y_test, rf_probs)
clsfier_fpr, clsfier_tpr, _ = roc_curve(y_test, clsfier_probs)


plt.plot(r_fpr, r_tpr, linestyle='--', label='Random prediction (AUROC = %0.3f)' % r_auc)
plt.plot(rf_fpr, rf_tpr, marker='.', label='Random Forest (AUROC = %0.3f)' % rf_auc)
plt.plot(clsfier_fpr, clsfier_tpr, marker='.', label='LR (AUROC = %0.3f)' % clsfier_auc)
...
print(f"Accuracy of Logistic Regression model equals : {accuracy}")
